...
# =============================== 解码部分 ======================================
#解码器隐藏层权重及偏置
P_W1 = tf.Variable(weight_variable([z_dim+n_classes, hidden_decoder_dim])) #[20,400]
...
#制作one_hot标签
label = tf.one_hot(y,depth=n_classes,on_value=1)
...
with tf.Session() as sess:
    ...
    for it in range(200001):
        ...
        _, loss = sess.run([train, vae_loss], feed_dict={X: batch_xs,Y:batch_ys})
        ...
        if it % 5000 == 0:
            ...
            print('-----------------------')
            batch_y = sess.run(label,feed_dict={y:i%10})
            for _ in range(16):
                a.append(batch_y)
            samples = sess.run(X_samples, feed_dict={z:np.random.randn(16, z_dim),Y:a})
